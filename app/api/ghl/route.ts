...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (contactId) {
      ...
      // Enroll into the nurture workflow so the follow-up automation fires.
      // Enrolling twice (started + completed) is fine: GHL rejects the
      // duplicate and we ignore it. Workflow ids belong to one sub-account, so
      // an unset var skips enrollment rather than calling another account's id.
